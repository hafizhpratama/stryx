//! Bench for `flow/command-injection-via-exec`.
//!
...
fn bench_bad(c: &mut Criterion) {
    ...
    c.bench_function("command_injection_via_exec/bad.ts", |b| {
...
fn bench_good(c: &mut Criterion) {
    ...
    c.bench_function("command_injection_via_exec/good.ts", |b| {
        b.iter(|| run_rules(black_box(&source), black_box(&path)))
    });
}
