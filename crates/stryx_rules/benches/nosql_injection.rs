//! Bench for `flow/nosql-injection`.
//!
...
//! recognised MongoDB collection-method shapes, followed by an
//! object-literal gate on the first argument.
...
fn bench_bad(c: &mut Criterion) {
    let path = fixtures_root().join("flow-nosql-injection/bad.ts");
    let source = std::fs::read_to_string(&path).expect("read bad.ts");
    c.bench_function("nosql_injection/bad.ts", |b| {
        b.iter(|| run_rules(black_box(&source), black_box(&path)))
    });
}

fn bench_good(c: &mut Criterion) {
    let path = fixtures_root().join("flow-nosql-injection/good.ts");
    let source = std::fs::read_to_string(&path).expect("read good.ts");
    c.bench_function("nosql_injection/good.ts", |b| {
        b.iter(|| run_rules(black_box(&source), black_box(&path)))
    });
}
