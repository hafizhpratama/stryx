//! Bench for `flow/insecure-deserialize`.
//!
//! Per AGENTS.md the per-rule per-file p99 budget is ≤ 1ms.
//! Slice 1 is single-file; the hot path is the call-expression
//! walk checking each call's callee against the small set of
//! recognised unsafe-deserialization shapes
//! (node-serialize `unserialize`, js-yaml `yaml.load`, Node
//! `vm.runInX`).
...
fn bench_bad(c: &mut Criterion) {
    let path = fixtures_root().join("flow-insecure-deserialize/bad.ts");
    let source = std::fs::read_to_string(&path).expect("read bad.ts");
    c.bench_function("insecure_deserialize/bad.ts", |b| {
        b.iter(|| run_rules(black_box(&source), black_box(&path)))
    });
}

fn bench_good(c: &mut Criterion) {
    let path = fixtures_root().join("flow-insecure-deserialize/good.ts");
    let source = std::fs::read_to_string(&path).expect("read good.ts");
    c.bench_function("insecure_deserialize/good.ts", |b| {
        b.iter(|| run_rules(black_box(&source), black_box(&path)))
    });
}
