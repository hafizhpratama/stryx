...
Thank you for considering contributing. Stryx grows by community-contributed
AI failure patterns — every Cursor/Claude Code/Copilot output that fooled you
is a potential rule that helps everyone else.

## Ways to contribute

In rough order of leverage:

1. **Report a rule request** — you saw AI generate dangerous code we don't
   yet catch. [Open a rule request](.github/ISSUE_TEMPLATE/new-rule-request.md)
   with the real output. This is the highest-impact contribution.
...
4. **Improve a rule's message or fix hint** — small, valuable, easy reviews.
...
Take a real Cursor / Claude Code / Copilot / v0 / Lovable output that
has the bug. **Do not invent examples** — synthetic bugs don't
represent real AI failure modes. Save the bad code in:
...
```ts
// Generated by Cursor + Claude Sonnet 4.6, 2026-04-12
// Prompt: "Build a Next.js API route that creates a user from request body"
```

### License compliance for fixtures (important)

Fixtures must be **your own AI-generated output** or material you have
explicit permission to redistribute under Apache 2.0:

- Generated yourself by prompting an AI tool — fine.
...
When in doubt, generate the fixture yourself with a fresh AI prompt
and document the prompt in the comment header.
...
- Why AI gets this wrong (the failure mode)
...
We're building a tool for everyone shipping AI-generated code, regardless
of experience level. Be kind.
