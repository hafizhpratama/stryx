...
When prompted with "create a debug endpoint that returns the current
config" or "add a `/api/health` endpoint with diagnostics," AI tools
commonly return the full `process.env` object or a `getConfig()`
helper that bundles every env var indiscriminately. The prompt asks
for "config"; the model interprets this as "all env vars."

The same shape appears with health checks, version endpoints, debug
panels, and admin dashboards. None of these are intentionally
malicious — they're tutorial-shaped scaffolds the model has seen
many times in benign contexts where the env vars happened to be
non-sensitive.
...
// Generated by Cursor + Claude Sonnet 4.6, 2026-04-18
// Prompt: "Create a debug endpoint that returns the current config"
