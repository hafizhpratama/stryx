...
When prompted with "create an auth wrapper for Next.js route handlers,"
AI coding tools commonly generate the *signature* of an auth wrapper
correctly — the right name, the right type parameters, the right
return shape — and skip the body, leaving a `TODO` comment or
returning the handler unchanged. The wrapper compiles, the route
imports it, and at every call site the code looks legitimately
protected.

We've seen this pattern most often when the prompt asks for a
type-safe wrapper without specifying the auth mechanism (NextAuth,
Clerk, Lucia, custom). The model produces the type-safe scaffolding
and defers the auth logic.
...
```ts
// File: lib/auth.ts
// Generated by Cursor + Claude Sonnet 4.6, 2026-04-15
// Prompt: "Create a type-safe auth wrapper for Next.js route handlers"
