...
AI coding tools (Cursor, Claude Code, GitHub Copilot, v0, Lovable)
commonly scaffold "factor the database call into a helper" without
also factoring in the validation step. The handler ends up reading the
request body, the helper ends up doing the DB write, and the
validation that *was* in the original tutorial pattern silently drops
out during the refactor — because the AI optimizes for compiling and
running, not for security boundaries.

The cross-file shape is what makes this hard to catch. Tutorial code
typically validates inline; AI tools then refactor that inline pattern
into a helper, and validation becomes nobody's responsibility.

We've seen this pattern in 60–80% of AI-generated CRUD endpoints in
real-world repos.
...
// Generated by Cursor + Claude Sonnet 4.6, 2026-04-20
// Prompt: "Next.js API route to create a user; factor the DB call into a helper"
