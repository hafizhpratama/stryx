...
AI coding tools produce raw-SQL escape-hatch calls when they
encounter requirements the typed API can't easily satisfy:
dynamic `ORDER BY` columns, dynamic `WHERE` clauses, full-text
search across multiple tables, optional join conditions. The
tutorial-code pattern is `pool.query(\`SELECT ... WHERE col = ${user.value}\`)`
or `prisma.$queryRawUnsafe(\`SELECT ... ${user.column}\`)` — it
compiles, it runs, the tests pass with their hardcoded inputs.
The threat model that says "the attacker controls `user.value`"
does not show up in training data because tutorials lean on
example inputs the author chose.

## Bad example (real AI output)

```ts
// Generated by Claude Code, 2026-05.
// Prompt: "Add a route that searches users by an arbitrary
//          ORDER BY column the caller specifies."
